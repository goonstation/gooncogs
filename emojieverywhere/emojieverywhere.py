...
class EmojiEverywhere(commands.Cog):
    ...
    @commands.command()
    @commands.is_owner()
    async def set_emoji_log(self, ctx: commands.Context, channel: Optional[discord.TextChannel]):
        if channel is None:
    ...
    async def steal_emoji(self, name, url):
        ...
        if emoji_log:
            if result:
                ...
                await emoji_log.send(f"Failed to steal emoji {name} {id}{' (animated)' if animated else ''}")
        return result
    ...
    def discord_emoji_from_url(self, url, name):
        match = re.match(r"https?://cdn\.discordapp\.com/emojis/([0-9]*)\.(gif|png)", url)
        if not match:
            return None
    ...
    async def emojitext(self, ctx: SlashContext, text: str):
        ...
        async def emoji_replace(emoji_name):
            stealable_urls = await self.config.custom("stealable_emoji", emoji_name).urls()
            emoji_strings = [(self.discord_emoji_from_url(url, emoji_name), url) for url, usable in stealable_urls if usable]
            if emoji_strings:
    ...
    @cog_slash(name="emoji",
            description="Posts an emoji with a given name."
            )
    async def emoji(self, ctx: SlashContext, emoji_name: str):
        try:
            ...
            if len(stealable_urls) > 0:
                # await ctx.defer()
                emoji = await self.steal_emoji(emoji_name, random.choice(stealable_urls)[0])
                if emoji:
                    if message_to_edit is not None:
            ...
            if self.bttv_enabled:
                # await ctx.defer()
                data = requests.get(f"https://api.betterttv.net/3/emotes/shared/search?query={emoji_name}&offset=0&limit=50").json()
                for emoji in data:
                    name = emoji['code']
                    if name == emoji_name:
    ...
    @commands.command()
    @commands.is_owner()
    async def snoop_for_emoji(self, ctx: commands.Context, msg: discord.Message, limit=100):
        found_count = 0
        async with ctx.typing():
    ...
    async def reroll_emoji(
        ...
        return await menu(ctx, [embed], controls, message=message, page=page, timeout=timeout)
    ...
    async def allemoji(self, ctx: commands.Context):
        html = ['<style>body{background:#333}div{display:inline-block;background:#222;color:#eee;font-family:Arial;width:200px;height:220px;margin:10px}img{height:200px;width:200px}</style>']
