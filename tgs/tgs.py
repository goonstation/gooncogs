...
class TGS(commands.Cog):
    ...
    async def get_bearer(self):
        if (
            self.bearer_expires is not None
            and self.bearer_expires > datetime.datetime.now(self.bearer_expires.tzinfo)
        ):
            return self.bearer
        if await self.login():
            return self.bearer
    ...
    async def server_restart(self, server):
        ...
        async with self.session.patch(
            self.host + "/DreamDaemon", headers={"Instance": str(server)}
        ) as res:
            return await self.process_response(res)
    ...
    async def server_diag(self, server):
        ...
        await self.assure_logged_in()
        async with self.session.patch(
            self.host + "/DreamDaemon/Diagnostics", headers={"Instance": str(server)}
        ) as res:
            return await self.process_response(res)
        ...
        async with ctx.typing():
            try:
                ...
                if response is None:
                    ...
                    await ctx.send(
                        f"{response['message']} (error code: {response['errorCode']})"
                    )
                else:
                    return response
                ...
                if e.status == 403:
                    await ctx.send(
                        f"Insuffiecient TGS user permissions (HTTP error code: {e.status})"
                    )
                elif e.status == 503:
                    await ctx.send(
                        f"TGS server is starting up or shutting down. (HTTP error code: {e.status})"
                    )
                elif e.status == 401:
                    await ctx.send(
                        f"Invalid TGS authentication, please contact bot owner. (HTTP error code: {e.status})"
                    )
                else:
                ...
                await ctx.send(
                    "Unable to login, please contact the bot owner and/or the TGS instance administrator."
                )
            except aiohttp.ClientConnectorError:
                await ctx.send(
                    "Unable to connect to the server, please contact the bot owner and/or the TGS instance administrator."
                )
        return None
    ...
    async def info(self, ctx: commands.Context, server: str):
        ...
        embed.description += (
            f"<t:{int(self._parse_iso_time(desc_timestamp).timestamp())}:F>"
        )
        embed.add_field(
            name="watchdog status",
            value=["Offline", "Restoring", "Online", "Delayed Restart"][res["status"]],
        )
        embed.add_field(
            name="byond version", value=res["activeCompileJob"]["byondVersion"]
        )
        commit_url = (
            res["activeCompileJob"]["repositoryOrigin"]
            + "/commit/"
            + res["activeCompileJob"]["revisionInformation"]["originCommitSha"]
        )
        commit_hash = res["activeCompileJob"]["revisionInformation"]["originCommitSha"][
            :7
        ]
        embed.add_field(name="commit", value=f"[{commit_hash}]({commit_url})")
        embed.set_footer(
            text=f"port: {res.get('currentPort', res.get('port', 'unknown'))}"
        )
        embed.timestamp = self._parse_iso_time(
            res["activeCompileJob"]["job"]["startedAt"]
        )
