...
RTT: {elapsed * 1000:.2f}ms"""
    ...
    async def _notes(
        self, ctx: commands.Context, ckey: str, clean=False, one_per_page=False
    ):
        goonservers = self.bot.get_cog("GoonServers")
        ckey = self.ckeyify(ckey)
        response = await goonservers.send_to_server_safe(
            "2", {"type": "getNotes", "ckey": ckey}, ctx.message
        )
        if response is None:
            return
        if response == 0:
        ...
        def add_field(name, value):
            ...
            for i, value_part in enumerate(
                ...
                if (
                    current_embed
                    and len(current_embed.fields) >= 25
                    or field_size + current_embed_size >= 5950
                ):
