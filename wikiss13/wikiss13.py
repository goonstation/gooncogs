...
class Wikiss13(commands.Cog):
    ...
    IGNORE_STUFF = re.compile(r'<table (style|class="wikitable mw-collapsible"|class="wikitable").*?</table>|<label.*?</label>|<div class="thumbcaption.*?</div></div>|<dl>.*?</dl>|<data value="summary hide">.*?</data>', re.DOTALL)
    ...
    async def generate_embed(self, page_json, session):
        ...
        async with session.get(
            ...
            if 'tofragment' in page_json:
                fragment_match = re.search(r'id="' + page_json['tofragment'] + r'".*?>', page_text)
                if fragment_match:
                    page_text = page_text[fragment_match.end():]
                else:
                    tab_matches = re.findall(r'<label class="tabs-label" for="tabs-input-[0-9]*-[0-9]*" data-tabpos="([0-9]*)">(.*?)</label>', page_text)
                    tab_id = None
                    ...
                    if tab_id is not None:
                        page_text = page_text.split(f'class="tabs-content tabs-content-{tab_id}">')[1]
                    else:
                        page_text = "ERROR"
        ...
        def format_desc(text):
            ...
            text = re.sub(r'<a[^>]*?href="(/.*?)"[^>]*?>(.*?)</a>', r'[\2](https://wiki.ss13.co\1)', text)
        ...
        embed.set_footer(
            text=text,
            icon_url=(
                "https://wiki.ss13.co/favicon.ico"
            ),
        )
        return embed
