...
class WorldTopic(commands.Cog):
    ...
    def iterable_to_params(self, iterable):
        if isinstance(iterable, str) or isinstance(iterable, int) or isinstance(iterable, float):
            return iterable
        result_parts = []
        for key in iterable:
            value = None
    ...
    @commands.command()
    @checks.is_owner()
    async def test_world_topic(self, ctx: commands.Context, addr_port: str, message: str):
